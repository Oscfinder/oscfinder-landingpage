...
const COMPARISON_ROWS: { label: string; values: [string, string, string] }[] = [
  { label: "Who it's for",      values: ["Solo founders starting outreach", "Growing sales teams", "Agencies and large teams"] },
  { label: "Scrapes / month",   values: ["50", "120", "300"] },
  { label: "Emails / month",    values: ["1,000", "3,000", "10,000"] },
  { label: "Exports / month",   values: ["20", "Unlimited", "Unlimited"] },
  { label: "Leads storage",     values: ["600", "3,000", "Unlimited"] },
  { label: "Users",             values: ["1", "3", "10"] },
  ...
  { label: "Onboarding",        values: ["Self-serve", "30-min setup call", "Full onboarding session"] },
];

// Derived from the same PLANS pricing used in the cards, so the two never drift apart.
const PRICING_SCHEDULE = TERMS.map((term) => ({
  term: term.label,
  values: PLANS.map((plan) => plan.price[term.key]) as [number, number, number],
}));

const COLUMN_LABELS = ["Starter", "Business ⭐", "Enterprise"];
